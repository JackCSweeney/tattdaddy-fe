...
RSpec.describe "Edit Artist Account Page", type: :feature do
  describe "As a logged-in user" do
    before do
      json_response_1 = File.read("spec/fixtures/artists/artist.json")
      json_response_2 = File.read("spec/fixtures/artists/identities.json")
    ...
    describe "shows a update profile form prefilled with user's" do
      it "name" do
        expect(page).to have_field("Name", with: "Tatto artists")
      end
    ...
    describe "redirects submitted update form to My Profile" do
      before do
        stub_request(:delete, "http://localhost:3000/api/v0/artist_identities")
          .with( body: {"{\"artist_identity\":{\"artist_id\":\"5\",\"identity_id\":\"6\"}}"=>nil})
